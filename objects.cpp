...
static void createWireCube()
{
	...
	// So ne Art Container für alle Buffer die mit Cube zu tun haben, für teekanne dann einen anderen
	//diese beiden buffer gehören zusammen
...
static void createCube()
{
	...
	glGenBuffers(1, &vertexbuffer); //könnte auch mehrere festlegen bsp: (3,array)
	//legt fest was mit buffer gemacht werden soll 
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
	//Daten werden vom adressraum der cpu bis auf grafikkarte uebertragen -> wie groß soll buffer sein, wo kommen daten her
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_vertex_buffer_data), g_vertex_buffer_data, GL_STATIC_DRAW);
	...
	//speicher auf grafikkarte für rgb werte
	glGenBuffers(1, &colorbuffer);
	//an speicher binden
	glBindBuffer(GL_ARRAY_BUFFER, colorbuffer);
	//daten reinschreiben
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_color_buffer_data), g_color_buffer_data, GL_STATIC_DRAW);
	//klar machen, dass daten in jeweiligen shader eingespeist werden --> Kanäle (=index) -> shader transformVertexShader datei (editor)

	//"Kanal" öffnen -> ermöglicht, dass es kanal 0 gibt
	glEnableVertexAttribArray(0); // Kein Disable ausführen !
